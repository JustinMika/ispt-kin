<?php
    ...
    if(!empty($_POST['p_depense']) && !empty($_POST['annee_acad']) && !empty($_POST['a_montant']) && !empty($_POST['faculte']) && !empty($_POST['promotion'])){
        ...
        $v = ConnexionBdd::Connecter()->prepare("SELECT * FROM depense_facultaire WHERE poste = ? AND id_section = ? AND id_annee = ?");
        $v->execute(array(strtolower($_POST['p_depense']), $_POST['faculte'], formater($_POST['annee_acad'])));
        $n = $v->rowCount();

        if($n <= 0){
            $insert = ConnexionBdd::Connecter()->prepare("INSERT INTO depense_facultaire(poste, montant, id_section, id_annee) VALUES(?,?,?,?)");
            ...
            if($ok){
                ...
                LogUser::addlog(VerificationUser::verif($_SESSION['data']['id_user']), 'a ajouté(e) un nouveau poste de dépense facultaire.');
            } else {
