        ...
        if(!empty($_POST['mat']) && !empty($_POST['promotion']) && !empty($_POST['frais']) && !empty($_POST['fac']) && !empty($_POST['annee_acad']) && !empty($_POST['montant_f'])){
            ...
            if(count($_POST['frais']) == count($_POST['montant_f'])){
                for ($i=0; $i < count($a) ; $i++) { 
                    ...
                    $insert = ConnexionBdd::Connecter()->prepare("DELETE FROM affect_fac_frais where matricule = ? AND promotion = ? AND faculte = ? AND annee_acad = ? AND type_frais = ? AND montant = ?");
                ...
                if(count($erreur) < 1){
                    echo 'ok';
                    LogUser::addlog(VerificationUser::verif($_SESSION['data']['noms']), 'il/elle a supprimé(e) certains type de frais facultaire affecter à '.$_POST['mat'].' : '.$_POST['promotion'].' : '.$_POST['fac']);
                }else{
