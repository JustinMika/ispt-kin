<?php
    ...
    isset($_POST['annee_acad_pay_etud']) && !empty($_POST['annee_acad_pay_etud'])){
        ...
        $pay_num_bordereau = strtoupper($_POST['pay_num_bordereau']);
        $date_p = $_POST['date_p'];
        // $date_p = $date_p;
        ...
        if($n > 0){
            ...
            if($n_result > 0){
                ...
                if($date_p <= date("Y-m-d")){
                    if($nbre <= 0){
                        // on verifie si le numeron du borderau n'existe pas encore dans la base de donnees
                        $verif = ConnexionBdd::Connecter()->prepare("SELECT * FROM payement WHERE num_borderon = ?");
                        $verif->execute(array($pay_num_bordereau));
                        ...
                        if($verif->rowCount() <= 0){
                            ...
                            if(intval($montant_t_a_payer + $pay_etud_p) <= intval($montant_t_p)){
                                ...
                                if($ok){
                                    echo ("Donnees insere");
                                    LogUser::addlog(VerificationUser::verif($_SESSION['data']['noms']), 'a effectué(e) un payement à partir du formulaire');
                                }else{
                            ...
                            // attention
                            die("/!\ ATTENSION : le numero de borderau : {$pay_num_bordereau} existe déjà pour un autre payement. renseignez-vous sur ce numéro svp...");
                        }
                    }else{
                ...
                die("le ".$type_frais." n'est pas affecté a l'étudiant(e) <b>[".$mat."-".$promotion."-".$fac."]</b>");
            }
        }else{
